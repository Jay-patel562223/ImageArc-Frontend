import SubscriptionList, { useSelectedOrder } from '@/components/subscription/subscription-list';
import Seo from '@/components/seo/seo';
import ErrorMessage from '@/components/ui/error-message';
import { useOrders } from '@/framework/order';
import { useSubscriptions } from '@/framework/subscription_package';
import Spinner from '@/components/ui/loaders/spinner/spinner';
import isEmpty from 'lodash/isEmpty';
import OrderDetails from '@/components/subscription/subscription-details';
import OrderListMobile from '@/components/subscription/subscription-list-mobile';
...
function NoOrderFound() {
  return (
    <div className="my-auto flex h-[80vh] w-full items-center justify-center rounded bg-light p-5 md:p-8">
      <NotFound text="text-no-subscription-found" />
    </div>
  );
}
  ...
  return (
    <>
      ...
      <div className="hidden w-full overflow-hidden lg:flex">
        ...
        {selectedOrder && (
          <OrderDetails
            order={ordersItem.find((order:any) => order._id === selectedOrder._id)!}
          />
        )}
      </div>
      <OrderListMobile

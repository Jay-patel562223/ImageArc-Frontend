...
export function useToggleWishlist(product_id: string) {
  ...
  return useMutation(client.wishlist.toggle);
...
export function useInWishlist({
  ...
  return {
    inWishlist: Boolean(newData) ?? false,
    isLoading,
    error,
    refetch,
  };
...
export function useInWishlistNew({
  enabled,
  product_id,
}: {
  product_id: string;
  ...
  const { data, isLoading, error, refetch } = useQuery<boolean, Error>(
    ...
    () => client.wishlist.checkIsInWishlistNew({ product_id }),
    {
      enabled,
    }
  );

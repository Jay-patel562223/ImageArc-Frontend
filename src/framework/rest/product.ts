...
export function useProductsCategory(options?: Partial<ProductQueryOptions>) {
  const { locale } = useRouter();
...
const formattedOptions = {
 ...
 const id = options?.id;
  ...
  const {
    ...
    [`${API_ENDPOINTS.PRODUCTSCATEGORY}/${id}`,formattedOptions],
    ({ queryKey, pageParam }) => {
    //  return client.products.PRODUCTSCATEGORY(Object.assign({}, queryKey[1], queryKey[0]));
     return client.products.PRODUCTSCATEGORY(Object.assign({}, queryKey[1], pageParam));
    },
    {
      getNextPageParam: ({ current_page, last_page }) =>
        last_page > current_page && { page: current_page + 1 },
...
export const useProductPrices = (
  ...
  const { data, isLoading, error } = useQuery(
    [API_ENDPOINTS.PRODUCTS_PRICES, formattedOptions],
    ({ queryKey }) =>
      client.products.PRODUCTS_PRICES()
  );

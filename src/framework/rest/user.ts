...
export function useUser() {
  ...
  if (data?.newStatus != 'active' && data?.newStatus != undefined) {
    Cookies.remove(AUTH_TOKEN_KEY);
    // Router.reload();
  }
...
export function useLogout() {
  ...
  async function handleLogout() {
    ...
    setAuthorized(false);
    resetCheckout();
    // localStorage.clear(CART_KEY)
    router.replace('/')
    await queryClient.refetchQueries(API_ENDPOINTS.USERS_ME);
...
export function useVerifyForgotPasswordToken() {
  const { actions } = useStateMachine({ updateFormState });
  const queryClient = useQueryClient();
  let [messages, setMessage] = useState<string | null>(null);
  let [formError, setFormError] = useState<any>(null);
  ...
  return { mutate, isLoading, messages, formError, setFormError, setMessage };
  // return { mutate, isLoading, formError, setFormError };
}

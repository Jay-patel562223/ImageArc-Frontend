  ...
  return (
    <div className="flex flex-col min-h-screen transition-colors duration-150 bg-gray-100">
      ...
      <MobileNavigation>
        <motion.button
          whileTap={{ scale: 0.88 }}
          onClick={() => setDisplayMobileHeaderSearch((prev) => {
            return true;
            // return !prev;
          })}
          className={cn("flex items-center justify-center h-full p-2 focus:outline-none focus:text-accent",router.asPath == '/' && __ ? 'text-accent' : '')}
        >

...
export default function MobileNavigation({
  ...
  const [__, setDisplayMobileHeaderSearch] = useAtom(
  ...
  const activeTab = (tab:any) => {
      return router.asPath == tab && !__ ? 'text-accent' : ''
  }
    ...
    <div className="visible h-12 md:h-14 lg:hidden">
      <nav className="fixed bottom-0 z-10 flex h-12 w-full justify-between bg-light py-1.5 px-2 shadow-400 ltr:left-0 rtl:right-0 md:h-14">
        <motion.button
          ...
          className={cn("flex h-full items-center justify-center p-2 focus:text-accent focus:outline-none")}
        >
        ...
        <motion.button
          ...
          className={cn("flex h-full items-center justify-center p-2 focus:text-accent focus:outline-none",activeTab('/'))}
        >
        ...
        <motion.button
          ...
          className={cn("product-cart relative flex h-full items-center justify-center p-2 focus:text-accent focus:outline-none",activeTab('/checkout'))}
        >

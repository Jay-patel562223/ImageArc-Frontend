...
export default function ReviewForm() {
  ...
  const { createReview, isLoading: creating,error:newError } = useCreateReview();

  const { updateReview, isLoading,error:newError1 } = useUpdateReview();
  const onSubmit = (
    values: Omit<
      CreateReviewInput,
    ...
    if (data?.my_review) {
      ...
      updateReview({
        ...values,
        // @ts-ignore
        photos: values?.photos.length != 0 ? values?.photos : '',
        // photos: values?.photos?.map(({ __typename, ...rest }) => rest),
        product_id: data.product_id,
        id: data.my_review._id,
    ...
    createReview({
      ...
      order_id: data.order_id,
      // photos: values?.photos.length != 0 ? values?.photos[0] : '',
      variation_option_id: data.variation_option_id,
    });
  };
  return (
    <div className="flex h-full min-h-screen w-screen flex-col justify-center bg-light md:h-auto md:min-h-0 md:max-w-[590px] md:rounded-xl">
      ...
      <div className="p-7">
        ...
        {/* <input type="file"  {...register('photos')}   className="mb-5"/> */}

              </div>
              {newError && (
                <div className="mt-3">
                  <ValidationError message={newError} />
                </div>
              )}
                {newError1 && (
                <div className="mt-3">
                  <ValidationError message={newError1} />
                </div>
              )}

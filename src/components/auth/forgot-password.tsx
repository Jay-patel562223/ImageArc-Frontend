...
function RenderFormSteps() {
  ...
  > = ({ password }) => {
    resetPassword({
      password, otp: state.token, email: state.email,
      token: ''
    });
  };
    ...
    <div>
      ...
      {state.step === 'Email' && (
        ...
        <>
          ...
          {/* {message && !formError ? 
          <Alert className="mb-4" variant='info' message={message} />
          : ''
        } */}
          {formError ?
            <Alert className="mb-4" variant='errorOutline' message={formError?.email} />
            : ''
          }
          <EmailForm
            email={state.email}
      ...
      {state.step === 'Token' && (
        <>
          {message && !tokenFormError ?
            <Alert className="mb-4" variant='info' message={message} />
            : ''
          }
          {messages && !tokenFormError ?
            <Alert className="mb-4" variant='info' message={messages} />
            : ''
          }
          {tokenFormError ?
            <Alert className="mb-4" variant='errorOutline' message={tokenFormError?.email} />
            : ''
          }
          {/* <Alert className="mb-4" variant={tokenFormError ? 'errorOutline' : 'info'} message={tokenFormError ? tokenFormError?.email : messages} /> */}

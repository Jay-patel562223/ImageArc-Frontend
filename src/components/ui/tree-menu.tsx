...
const TreeMenuItem: React.FC<TreeMenuItemProps> = ({
  ...
  const isActive =
    active === item.name ||
    item?.children?.some((_item: any) => _item.name === active);
...
function TreeMenu({ items, className }: TreeMenuProps) {
  ...
  const allData = [{_id:'',name:'All',image:''},...items];
  // unshift

  return (
    <ul className={cn('text-xs', className)}>
      <h1 style={{fontSize: 'x-large',marginBottom:'15px',fontWeight: 'bold',marginTop:'10px'}}>Categories</h1>
      {/* <h1 style={{fontSize: 'large',textDecoration: 'underline',marginBottom:'15px'}}>Categories</h1> */}
      ...
      {allData?.map((item: any) => (
        <TreeMenuItem key={item?._id} item={item} />

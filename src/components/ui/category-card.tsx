...
const CategoryCard: React.FC<CategoryItemProps> = ({ item, onClick }) => {
  ...
  return (
    <div
      className="relative w-full h-60 rounded-lg p-8 bg-light shadow-downfall-sm transition-shadow hover:shadow-downfall-lg group"
      onClick={onClick}
      role="button"
    >
      <div className="flex flex-col flex-1 h-full relative z-10">
        <h3 className="text-heading font-semibold text-lg mb-1" style={{color:'white'}}>{item?.name}</h3>
      ...
      <div className="absolute bottom-0 ltr:right-0 rtl:left-0 w-full h-full rounded-lg overflow-hidden" style={{display: 'grid'}}>
        <Image

...
const Helium: React.FC<HeliumProps> = ({ product, className }) => {
  ...
  const { _id,name, image, unit, quantity, min_price, max_price,sale_price,price, product_type } =
    product ?? {};
  ...
  const { price:prices, basePrice, discount } = usePrice({
    amount: Number(sale_price) ? Number(sale_price) : Number(price!),
    baseAmount: Number(sale_price) ? Number(sale_price) : Number(price!),
  });

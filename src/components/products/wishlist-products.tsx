...
import AddToCartWishlistBtn from './add-to-cart/add-to-cart-wishlist-btn';
import { DEFAULT_LANGUAGE } from '@/lib/constants';
import { AddToCartWishlist } from './add-to-cart/add-to-cart-wishlist';
...
function WishlistItem({ product }: { product: Product }) {
  ...
  const productSingleUrl = Routes.product(product._id);
  ...
  const { price: minPrice } = usePrice({
    amount: Number(product?.min_price),
  });
  const { price: maxPrice } = usePrice({
    amount: Number(product?.max_price),
  });
  ...
  return (
    <div className="flex w-full items-start space-x-4 border-b border-gray-200 py-5 first:pt-0 last:border-0 last:pb-0 rtl:space-x-reverse sm:space-x-5 xl:items-center">
      ...
      <div className="flex w-full flex-col items-start sm:flex-row sm:justify-between sm:space-x-4 rtl:sm:space-x-reverse xl:items-center">
        ...
        <div className="mt-4 flex w-full flex-col justify-between space-y-3 xs:flex-row xs:space-y-0 sm:w-auto sm:flex-col sm:justify-end sm:space-y-3 md:mt-0" style={{width: 'inherit'}}>
          ...
          <div className="flex items-center space-x-6 rtl:space-x-reverse sm:justify-end" >
            ...
            <button
              ...
              //@ts-ignore
              onClick={() => removeFromWishlist(product?.wishlist[0]?._id)}
              disabled={isLoading}
            >
              {t('text-remove')}
            </button>
